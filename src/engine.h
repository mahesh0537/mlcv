...
public:
    ...
    bool loadNetwork();
    // Run inference.
    bool runInference(const std::vector<cv::Mat>& inputFaceChips, std::vector<cv::Rect>& featureVectors, int img_w, int img_h);
private:
    ...
    samplesCommon::ManagedBuffer m_outputBuff_bbox;
    samplesCommon::ManagedBuffer m_outputBuff_score;
